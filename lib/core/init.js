RESUtils.init = function() {
	...
	if (
			...
			(/(?:static|thumbs|blog|code)\.discussion\.mit\.edu/i.test(location.hostname)) ||
			(/^[www\.]?(?:i|m)\.discussion\.mit\.edu/i.test(location.href)) ||
...
RESUtils.init._localStorageFailed = function(error) {
	...
	var RESFail = 'Sorry, but localStorage seems inaccessible. MIT Discussion Enhancement Suite can\'t work without it. \n\n';
	if (BrowserDetect.isSafari()) {
		RESFail += 'Since you\'re using Safari, it might be that you\'re in private browsing mode, which unfortunately is incompatible with MDES until Safari provides a way to allow extensions localStorage access.';
