addModule('backupAndRestore', function(module, moduleID) {
	...
	module.description = 'Backup and restore your MIT Discussion Enhancement Suite settings.';
	module.options = {
		backup: {
			type: 'button',
			text: 'backup',
			callback: backup,
			description: 'Download a backup of your current MDES settings.'
		},
		restore: {
			type: 'button',
			text: 'restore',
	...
	function restoreFromFile(e) {
		...
		if (file.name.search(/\.resbackup$/) !== -1) {
			...
			reader.onload = function(e) {
				...
				alert('Your RES settings have been imported. Reloading MIT Discussion.');
				location.hash = '';
