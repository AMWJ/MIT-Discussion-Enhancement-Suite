addModule('userTagger', function(module, moduleID) {
	...
	function attachVoteHandlers(obj, maxPairs) {
		...
		voteButtons.forEach(function (voteButton) {
			//$(voteButton).clone().prop("onclick","alert('yo')").text("yo").insertAfter($(voteButton));
		});
		// Event to capture vote state if a click is completed.
		voteButtons.forEach(function (voteButton) {
			voteButton.addEventListener('mousedown', saveVoteState, true);
		});
		voteButtons.forEach(function (voteButton) {
			voteButton.addEventListener('click', handleVoteClick, true);
		});
	}
	function saveVoteState(e) {
		// Computes the needed vote modification needed if the arrow is clicked.
		var $this = $(this);
	...
	function handleVoteClick(e) {
		...
		/* MDES: But not on MIT Discussion! In MIT Discussion, onclick happens after
		 * the class changes, so we need an event that captures the vote modification needed
		 * during mousedown, to be applied in this method, if the click was completed.
		 * The mousedown event is saveVoteState(), above.
		 */
		/*if ($this.hasClass('up')) {
			// adding an upvote
			votes++;
	...
	module.usernameSelector = '.contents .author, .noncollapsed a.author, p.tagline a.author, #friend-table span.user a, .sidecontentbox .author, div.md a[href^="/u/"]:not([href*="/m/"]), div.md a[href*="discussion.mit.edu/u/"]:not([href*="/m/"]), .usertable a.author, .usertable span.user a, div.wiki-page-content .author, .commentingAsUser';
