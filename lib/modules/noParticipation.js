addModule('noParticipation', function(module, moduleID) {
	...
	module.include = [
		/^https?:\/\/(?:.*\.)?(?:\w+-)?np(?:-\w+)?\.discussion\.mit\.edu\/*/i  // np.discussion.mit.edu, np-nm.discussion.mit.edu, nm-np.discussion.mit.edu, www.np.discussion.mit.edu, www.np-nm.discussion.mit.edu
	];
	...
	var npHrefRegex = /^https?:\/\/(?:[\w\.]*)np\.discussion\.mit\.edu\//;
	var removeNpRegexp = /^https?:\/\/(?:[\w\.]*)?discussion\.mit\.edu(.*)/;
	var replaceNpWith = document.location.protocol + '//discussion\.mit\.edu';
